...
object Importer extends App with Complements {
  ...
  Source(1 to 100) // TODO toplam pokemon sayısı / limit
    .mapAsync(1) { count =>
      ...
      val x = clientResponse.map(clientResponse =>
        clientResponse.results.map { pokemonInfo =>
          for {
            ...
            res = pokemonDetail.types.map(res => pokemonRepo.insertTypes(TypeDto(pokemonDetail.id, res.`type`.name)))
          } yield Future.sequence(res)
      })
